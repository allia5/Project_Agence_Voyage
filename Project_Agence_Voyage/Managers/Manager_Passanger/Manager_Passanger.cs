    ...
    public class Manager_Passanger : IManager_Passanger
    {
        ...
        public int post_passanger(Passanger passanger)
        {
            try
            {
                SqlConnection connection = new SqlConnection(cnStr);
                SqlCommand cmd = new SqlCommand(
                    "INSERT INTO [dbo].[Passanger]([id_passanger],[nom],[prenom],[sexe],[date_naissance],[nationalitè],[passport_num],[email],[id_client])   VALUES (@id_passanger,@Nom,@prenom,@sexe,@date_naissance,@nationalitè,@passport_num,@email,@idClient)", connection);
                ...
                cmd.Parameters.AddWithValue("@idClient", passanger.idClient);
